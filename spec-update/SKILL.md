...
响应处理：
- `spec_confirm` 成功返回确认 → 继续；工具已自动把文档 frontmatter 的 `status` 更新为 `已确认`，Agent 不得再手工查找/替换 `status: 未确认`。
- `spec_confirm` 超时或失败 → 回退原生确认；仅当用户确认通过时，Agent 才手工将 `status` 更新为 `已确认`。
- 选择修改/修复或"Other" → 根据用户反馈调整，保持或改回 `status: 未确认`，然后重新确认。
